...
const MusicTheoryComponent: React.FC = () => {
            ...
            <ul className="space-y-2">
                {Object.keys(musicData.key_elements).map((key) => (
                    <li key={key}>
                        ...
                        <p className="text-gray-300 mb-1">{musicData.key_elements[key].description}</p>
                        <p className="italic text-gray-500">{musicData.key_elements[key].quote}</p>
                    </li>
                ))}
            </ul>
        </section>
        ...
        <section>
            ...
            <ul className="space-y-2">
                {Object.keys(musicData.additional_topics).map((key) => (
                    <li key={key}>
                        <strong className="block text-lg font-bold">{key}</strong>
                        <p className="text-gray-300 mb-1">{musicData.additional_topics[key].description}</p>
                        <p className="italic text-gray-500">{musicData.additional_topics[key].quote}</p>
                    </li>
                ))}
            </ul>
        </section>
